...
TAKEN FROM MY DATASETANALYSIS PACKAGE
...
def bivariate(x, y, xscale, yscale, xlabel, ylabel, draw_trend = False, trend_bg_color="#E5FFE5", point_color="#005500", yeqx = True, title="", color_delta = True, fname = "default.pdf"):
    ...
    if draw_trend:
        ...
        print(pcc)
        if yeqx:
            plt.plot(lx,lx, color="black")
    if color_delta:
        # relative to 
        d = np.sqrt(np.power((y-x) / 0.5253219, 2))
        _, x_max = xscale
        d_normalized = d / (x_max / 0.5253219)
        plt.scatter(x,y, c=d_normalized, marker=".", s=15)
    else:
    ...
    if title != "":
        ...
        top = 0.97
    plt.subplots_adjust(left=0.165, bottom=0.162, right=0.97, top=top, wspace=None, hspace=None)
    #plt.show()
